...
const weeks = [
  {
    week: 1, stage: "建立全景", group: "foundation", title: "AI 系统全栈", book: "书籍第 1 章", reading: "AIInfra 00Summary", outcome: "画出芯片、编译器、框架、训练与推理的全栈关系。",
  ...
  {
    week: 2, stage: "建立全景", group: "foundation", title: "计算模式与性能", book: "书籍第 2 章", reading: "MFU 与性能建模", outcome: "估算 Transformer Block 的参数量、FLOPs、显存与算术强度。",
  ...
  {
    week: 3, stage: "硬件与集群", group: "foundation", title: "CPU、GPU 与 AI 芯片", book: "书籍第 3 章", reading: "AICluster Roadmap", outcome: "比较四类芯片的执行模型、存储层次和适用负载。",
  ...
  {
    week: 4, stage: "硬件与集群", group: "foundation", title: "GPU、互联与通信", book: "书籍第 4 章", reading: "StorComm", outcome: "推导 Ring AllReduce 通信量，厘清 NVLink、PCIe 与 RDMA。",
  ...
  {
    week: 5, stage: "硬件与集群", group: "foundation", title: "TPU、NPU 与异构计算", book: "书籍第 5–7 章", reading: "SuperPod", outcome: "解释脉动阵列、数据流、布局转换和集群扩展。",
  ...
  {
    week: 6, stage: "编译与算子", group: "compiler", title: "传统编译器与 IR", book: "书籍第 8–9 章", reading: "编译器基础", outcome: "追踪一个简单函数从前端到 IR 再到目标代码的路径。",
  ...
  {
    week: 7, stage: "编译与算子", group: "compiler", title: "图优化与后端优化", book: "书籍第 10–11 章", reading: "TrainAcceler", outcome: "手工完成融合、常量折叠、CSE 与死代码消除示例。",
  ...
  {
    week: 8, stage: "编译与算子", group: "compiler", title: "CUDA、CANN 与 Ascend C", book: "书籍第 12–13 章", reading: "算子开发", outcome: "实现或分析向量加法 Kernel，记录线程与内存映射。",
  ...
  {
    week: 9, stage: "推理系统", group: "inference", title: "推理系统与大模型推理", book: "书籍第 14 章", reading: "05Infer", outcome: "比较 prefill/decode，计算 KV Cache 的增长与瓶颈。",
  ...
  {
    week: 10, stage: "推理系统", group: "inference", title: "压缩、量化与转换", book: "书籍第 15–17 章", reading: "CompDistill", outcome: "比较 FP32 与 INT8/INT4 的模型大小、精度和延迟。",
  ...
  {
    week: 11, stage: "推理系统", group: "inference", title: "图优化与 Kernel 优化", book: "书籍第 18–19 章", reading: "推理与训练加速", outcome: "比较直接卷积、Im2Col 与 Winograd 的数据流。",
  ...
  {
    week: 12, stage: "框架与分布式", group: "framework", title: "框架与自动微分", book: "书籍第 20–21 章", reading: "Mini Transformer 基础", outcome: "从零实现标量级反向模式自动微分并验证梯度。",
  ...
  {
    week: 13, stage: "框架与分布式", group: "framework", title: "计算图", book: "书籍第 22 章", reading: "Mini Transformer", outcome: "追踪模型的前向图、反向图、调度和控制流限制。",
  ...
  {
    week: 14, stage: "框架与分布式", group: "framework", title: "分布式并行", book: "书籍第 23 章", reading: "ParallelBegin / ParallelAdv", outcome: "选择 DP、TP、PP、SP、EP 与 ZeRO，并解释权衡。",
  ...
  {
    week: 15, stage: "综合项目", group: "framework", title: "大模型专项补全", book: "回顾书籍薄弱覆盖部分", reading: "存储、云平台、MoE", outcome: "整理至少 20 个书籍之外的大模型工程知识点。",
  ...
  {
    week: 16, stage: "综合项目", group: "framework", title: "综合项目", book: "全书与知识地图复盘", reading: "选择相关 AIInfra 模块", outcome: "交付可运行代码、实验数据、图表与 2000 字复盘。",
  ...
  list.innerHTML = weeks.map((item) => {
    ...
    return `
      <article class="week-card ${isWeekDone(item.week) ? "is-complete" : ""}" data-group="${item.group}" data-week-card="${item.week}">
        ...
        <div class="week-title"><strong>${item.title}</strong><span>${item.stage} · ${item.book}</span></div>
...
function getTaskDetail(item, taskIndex) {
  ...
  if (taskIndex === 0) return { ...common, title: `精读：${item.book}`, description: `围绕“${item.outcome}”精读本周书籍内容。`, items: ["标记至少 3 个核心概念", "记录 2 个关键权衡", "写下 1 个仍需验证的问题"], deliverable: "在笔记中留下章节摘要、概念关系与疑问。" };
...
function openTaskDetails(weekNumber, taskIndex) {
  ...
  $("#taskLinks").innerHTML = (detail.links || []).map(([label, path]) => `<a href="${infraUrl(path)}" target="_blank" rel="noreferrer"><span>${label}</span><small>${path}</small><b aria-hidden="true">↗</b></a>`).join("");
